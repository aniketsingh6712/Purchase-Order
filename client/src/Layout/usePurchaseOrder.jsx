...
const usePurchaseOrders = () => {
  ...
  const fetchPurchaseOrders = async () => {
    try {
      ...
      if (Array.isArray(dummyPurchaseOrders.data)) {
        setApprovedPOs(dummyPurchaseOrders.data.filter((po) => po.status === "APPROVED"));
        setRejectedPOs(dummyPurchaseOrders.data.filter((po) => po.status === "REJECTED"));
      } else {
        setApprovedPOs([]);
        setRejectedPOs([]);
      }
  ...
  return { approvedPOs, rejectedPOs, loading, error, refresh: fetchPurchaseOrders };
};
