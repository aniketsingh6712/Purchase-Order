...
function UserDashboard() {
 ...
 const fetchPOs = useCallback(async () => {
  ...
  // 🧾 When form is submitted successfully, refresh drafts
  const submitPurchaseOrder = async () => {
    try {
      await axios.post(
        "http://localhost:3001/api/purchase/po",
        formData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      await fetchPOs(); // ✅ refresh all POs, including drafts
    } catch (err) {
